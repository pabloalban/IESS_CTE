...
pib_real <- read_excel(file,                                  
                       ...
                       skip = 0) %>% clean_names() %>%
  dplyr::select( anio,
                 pib := pib_usd_a_precios_actuales,
                 pib_constantes := pib_usd_a_precios_constantes,
                 crecimiento_pib )
...
rendimiento_biess <- read_excel( file,
                                 ...
                                 na = "",
                                 skip = 0 ) %>% clean_names( ) %>%
  mutate( fecha = as.Date( fecha, "%d/%m/%Y", tz = parametros$time_zone ) )
...
save( inflacion,
      ...
      rendimiento_biess,
      tasas_macro,
      file = paste0( parametros$RData_macro, 'IESS_contexto_economico.RData' ) )
