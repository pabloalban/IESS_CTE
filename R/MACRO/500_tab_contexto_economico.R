...
#Parámetros-----------------------------------------------------------------------------------------
anio_fin <- parametros$anio_ini + 40
anio_ini <- parametros$anio
anio_corte <- parametros$anio_ini
...
#Tabla de PIB---------------------------------------------------------------------------------------
aux <- pib_real %>%
  filter( anio >= '2000', anio <= anio_corte ) %>%
...
aux <- salarios %>%
  ...
  na.omit( . ) %>%
  dplyr::select( periodo, sal_prom, incremento, tasa ) %>%
  mutate( periodo = as.character( format( periodo, "%b %Y" ) ) )
...
aux <- tasas_interes %>%
  ...
  mutate( periodo = as.character( format( periodo, "%b %Y" ) ) ) %>%
  dplyr::select( periodo, tasa_activa, tasa_pasiva, spread )
...
aux <- rendimiento_biess %>%
  filter( fecha <= as.Date( "31/12/2022", "%d/%m/%Y" ), mes == '12' ) %>%
...
aux_1 <- tasas_macro_anuales %>%
  dplyr::select( anio, t_sal ) %>%
  mutate( anio = as.character( anio ) )

aux <- predicciones_anuales %>%
  filter( anio >= anio_corte + 1, anio <= anio_fin ) %>%
  mutate( anio = as.character( anio ), pib_anual = pib_anual / 1000 ) %>%
  left_join( ., aux_1, by = 'anio' ) %>%
  dplyr::select( anio, pib_anual, tp_anual, t_sal, sbu_anual, ipc_dic, inf_anual )
...
aux_xtab <- xtable( aux, digits = c( 0, 0, rep( 5, ncol( aux ) - 1 ) ) )
...
aux_xtab <- xtable( aux_xtab, digits = c( 0, 0, rep( 2, ncol( aux ) - 1 ) ) )
print( 
  aux_xtab,
  file = paste0( 
    parametros$resultado_tablas,
    'iess_ma_multi_p_valores',
    '.tex'
  ),
  type = 'latex',
  include.colnames = FALSE,
...
sigma_diff_pib <- format( 
  round( modelo$diff_pib$sigma, 4 ),
  nsmall = 3,
  decimal.mark = ",",
  big.mark = "."
)
...
f_diff_pib <- format( 
  round( modelo$diff_pib$fstatistic[1], 4 ),
  nsmall = 3,
  decimal.mark = ",",
  big.mark = "."
)
...
f_value_diff_pib <- pf( 
  modelo$diff_pib$fstatistic[1],
  modelo$diff_pib$fstatistic[2],
  modelo$diff_pib$fstatistic[3],
  lower.tail = FALSE
)


f_value_diff_pib <- if_else( 
  f_value_diff_pib < 0.001,
  " <\\,0,001",
  format( 
    round( f_value_diff_pib, 4 ),
    nsmall = 3,
    decimal.mark = ",",
    big.mark = "."
  )
...
sigma_diff_tasa_pasiva <- format( 
  round( modelo$diff_tasa_pasiva$sigma, 4 ),
  nsmall = 3,
  decimal.mark = ",",
  big.mark = "."
)
...
f_diff_tasa_pasiva <- format( 
  round( modelo$diff_tasa_pasiva$fstatistic[1], 4 ),
  nsmall = 3,
  decimal.mark = ",",
  big.mark = "."
)

f_value_diff_tasa_pasiva <- pf( 
  modelo$diff_tasa_pasiva$fstatistic[1],
  modelo$diff_tasa_pasiva$fstatistic[2],
  modelo$diff_tasa_pasiva$fstatistic[3],
  lower.tail = FALSE
)


f_value_diff_tasa_pasiva <- if_else( 
  f_value_diff_tasa_pasiva < 0.001,
  " <\\,0,001",
  format( 
    round( f_value_diff_tasa_pasiva, 4 ),
    nsmall = 3,
    decimal.mark = ",",
    big.mark = "."
  )
...
sigma_diff_sal_prom <- format( 
  round( modelo$diff_sal_prom$sigma, 4 ),
  nsmall = 3,
  decimal.mark = ",",
  big.mark = "."
)
...
f_diff_sal_prom <- format( 
  round( modelo$diff_sal_prom$fstatistic[1], 4 ),
  nsmall = 3,
  decimal.mark = ",",
  big.mark = "."
)

f_value_diff_sal_prom <- pf( 
  modelo$diff_sal_prom$fstatistic[1],
  modelo$diff_sal_prom$fstatistic[2],
  modelo$diff_sal_prom$fstatistic[3],
  lower.tail = FALSE
)

f_value_diff_sal_prom <- if_else( 
  f_value_diff_sal_prom < 0.001,
  " <\\,0,001",
  format( 
    round( f_value_diff_sal_prom, 4 ),
    nsmall = 3,
    decimal.mark = ",",
    big.mark = "."
  )
...
sigma_diff_sbu <- format( 
  round( modelo$diff_sbu$sigma, 4 ),
  nsmall = 3,
  decimal.mark = ",",
  big.mark = "."
)
...
f_diff_sbu <- format( 
  round( modelo$diff_sbu$fstatistic[1], 4 ),
  nsmall = 3,
  decimal.mark = ",",
  big.mark = "."
)
...
f_value_diff_sbu <- pf( 
  modelo$diff_sbu$fstatistic[1],
  modelo$diff_sbu$fstatistic[2],
  modelo$diff_sbu$fstatistic[3],
  lower.tail = FALSE
)


f_value_diff_sbu <- if_else( 
  f_value_diff_sbu < 0.001,
  " <\\,0,001",
  format( 
    round( f_value_diff_sbu, 4 ),
    nsmall = 3,
    decimal.mark = ",",
    big.mark = "."
  )
...
sigma_diff_ipc <- format( 
  round( modelo$diff_ipc$sigma, 4 ),
  nsmall = 3,
  decimal.mark = ",",
  big.mark = "."
)
...
f_diff_ipc <- format( 
  round( modelo$diff_ipc$fstatistic[1], 4 ),
  nsmall = 3,
  decimal.mark = ",",
  big.mark = "."
)
...
f_value_diff_ipc <- pf( 
  modelo$diff_ipc$fstatistic[1],
  modelo$diff_ipc$fstatistic[2],
  modelo$diff_ipc$fstatistic[3],
  lower.tail = FALSE
)


f_value_diff_ipc <- if_else( 
  f_value_diff_ipc < 0.001,
  " $<\\,0,001$",
  format( 
    round( f_value_diff_ipc, 4 ),
    nsmall = 3,
    decimal.mark = ",",
    big.mark = "."
  )
...
print( 
  ...
  add.to.row =
    list( 
      pos = list( 3, 4, 6, 10, 12 ),
      #, 4, 6, 10, 12 ), # posición después de 3, 4, 6, 10, 12
      command = c( 
        paste( 
          " \\hline \n",
        ...
        paste( 
          "\\hline \n \\multicolumn{5}{l}{Error estándar de los residuos: ",
          sigma_diff_tasa_pasiva,
          " sobre ",
          df_diff_tasa_pasiva[2],
          " grados de libertad} \\\\ \n",
          "\\multicolumn{5}{l}{Estadístico $F$: ",
          f_diff_tasa_pasiva,
          " sobre ",
          df_diff_tasa_pasiva[1],
          " y ",
          df_diff_tasa_pasiva[2] ,
          " grados de libertad, p-valor",
          f_value_diff_tasa_pasiva,
          "}",
...
aux <- coeficientes_biess %>%
  mutate( var = c( '$AR( 1 )$', '$AR( 2 )$' ) )

sigma_modelo <- format( 
  round( se_mod_biess, 4 ),
  nsmall = 3,
  decimal.mark = ",",
  big.mark = "."
)
