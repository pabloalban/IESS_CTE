...
aux <- inflacion %>%
  filter( anio >= '2003',
          anio <= paste0( parametros$anio_ini) ) %>%
...
iess_inflacion  <- ggplot( data = aux, aes( x = periodo ) ) +
  ...
  geom_line( aes( y = ipc,
             ...
             linewidth = graf_line_size ) +
  scale_x_date( 
    breaks = seq( as.Date( '2002-12-01' ), as.Date( '2024-12-01' ), by = '24 months' ),
    date_labels = '%b %Y',
    limits = as.Date( c( '2002-12-01', '2024-12-01' ), '%Y-%m-%d' )
  ) +
  scale_y_continuous( 
    breaks = y_brk,
    labels = y_lbl,
...
iess_desempleo  <- ggplot( data = aux, aes( x = periodo ) ) +
  ...
  geom_line( aes( y = desempleo_nacional,
             ...
             linewidth = graf_line_size ) +
  scale_x_date( 
    breaks = seq( as.Date( '2007-12-01' ), as.Date( '2024-12-01' ), by = '12 months' ),
    date_labels = '%b %Y',
    limits = as.Date( c( '2007-12-01', '2024-12-01' ), '%Y-%m-%d' )
  ) +
  scale_y_continuous( 
    breaks = y_brk,
    labels = y_lbl,
...
aux <- sbu %>%
  filter( anio <= paste0( parametros$anio_ini ) )

x_lim <- c( 2000, parametros$anio_ini )
...
x_lim <- c( 2005,  parametros$anio_ini )
...
iess_salarios_evo <- ggplot( data = aux,
  ...
  scale_x_date( 
    breaks = seq( as.Date( '2006-12-01' ), as.Date( '2022-12-01' ), by = '12 months' ),
    date_labels = '%b %Y',
...
iess_pib_real <- ggplot( data = aux,
  ...
  scale_x_date( 
    breaks = seq( as.Date( '2000-01-01' ), as.Date( '2024-01-01' ), by = '24 months' ),
    date_labels = '%Y',
    limits = as.Date( c( '2000-0-01', '2023-12-31' ), '%Y-%m-%d' )
  ) +
  scale_y_continuous( 
...
biess_rendimiento <- ggplot( data = df_bar,
  ...
  scale_linetype_manual( NULL, values = 1 ) +
  scale_x_date( 
    breaks = seq( as.Date( '2011-12-01' ), as.Date( '2022-12-01' ), by = '12 months' ),
    date_labels = '%b %Y',
...
iess_pib_pred <- ggplot(  ) +
  ...
  scale_y_continuous( breaks = brks_y,
                      labels = lbls_y,
                      limits = lim_y ) +
  scale_x_continuous( labels = seq( 2002, anio_fin, by = 5 ),
                      breaks = seq( 2002, anio_fin, by = 5 ) ) +
  theme_bw(  ) +
  plt_theme
...
aux_his <- aux %>% filter(  fecha <= as.Date( '2024-12-01' ) )
aux_pred <- aux %>% filter( fecha >= as.Date( '2024-12-01' ), year( fecha ) <= anio_fin )
...
iess_tp_pred <- ggplot(  ) +
  ...
  scale_y_continuous( breaks = brks_y,
                      ...
                      limits = lim_y ) +
  scale_x_date( 
    breaks = seq( as.Date( '2002-12-01' ), as.Date( paste0( anio_fin,'-12-01' ) ), by = '60 months' ),
    date_labels = '%b %Y',
    limits = as.Date( c( '2002-12-01', paste0( anio_fin,'-12-01' ) ), '%Y-%m-%d' )
  ) +
  theme_bw(  ) +
  plt_theme  +
  theme( axis.text.x = element_text( 
...
biess_rendimiento_pred <- ggplot(  ) +
  ...
  scale_y_continuous( breaks = brks_y,
                      ...
                      limits = lim_y ) +
  scale_x_date( 
    breaks = seq( as.Date( '2012-12-01' ), as.Date( paste0( anio_fin,'-12-01' ) ), by = '60 months' ),
    date_labels = '%b %Y',
    limits = as.Date( c( '2012-12-01', paste0( anio_fin,'-12-01' ) ), '%Y-%m-%d' )
  ) +
  theme_bw(  ) +
  plt_theme  +
  theme( axis.text.x = element_text( 
